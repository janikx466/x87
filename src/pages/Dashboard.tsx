...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(
      ...
      (error) => {
        ...
        setVaultsLoading(false); // 🔥 prevent infinite loader
      }
    );
  ...
  const inviteLink = `${window.location.origin}/auth?ref=${safeUserData.inviteCode}`;
  ...
  const handleShare = () => {
    if (navigator.share) {
      navigator.share({
        title: "Join SecretGPV",
        text: "Share photos privately!",
        url: inviteLink,
      });
    } else handleCopy();
  };

  // 🔥 ONLY loading check (FIXED)
        ...
        <div className="flex items-center justify-between px-4 py-3">
          <div className="flex items-center gap-2">
            ...
            <span className="font-bold text-lg">
              Secret<span className="text-primary">GPV</span>
            </span>
          </div>
      ...
      <div className="px-4 pt-4 space-y-6">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="bg-card border border-border rounded-xl p-4">
            ...
            <p className="font-bold text-primary">{safeUserData.planName}</p>
          </div>

          <div className="bg-card border border-border rounded-xl p-4">
            <p className="text-xs text-muted-foreground">Credits</p>
            <p className="font-bold text-primary">{safeUserData.credits}</p>
          </div>
          ...
          <div className="bg-card border border-border rounded-xl p-4">
            ...
            <p className="font-bold text-sm">
              {safeUserData.planExpiry
                ? safeUserData.planExpiry.toLocaleDateString()
                : "N/A"}
            </p>
          </div>
        </div>
        ...
        <div className="bg-card border border-border rounded-xl p-4">
          ...
          <div className="flex gap-2">
            <div className="flex-1 bg-secondary rounded-lg px-3 py-2 text-xs truncate">
              {inviteLink}
            </div>
